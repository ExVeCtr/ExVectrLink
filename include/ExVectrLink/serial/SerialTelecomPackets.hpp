...
public:
  ...
  uint16_t remoteDesyncCount = 0; ///< The far ends FHSS desync counter.

  SerialPacketType getPacketType() const { return SerialPacketType::LinkInfo; }
  // 6 bytes × 2 sides + 1 valid flag + 4 bytes time + 2 bytes desyncs.
  uint8_t numBytes() const { return 19; }
  ...
  void serialize(uint8_t *buffer) const {
    ...
    buffer[17] = (uint8_t)(remoteDesyncCount & 0xFF);
    buffer[18] = (uint8_t)((remoteDesyncCount >> 8) & 0xFF);
  }
  bool deserialize(const uint8_t *buffer) {
    ...
    remoteDesyncCount = static_cast<uint16_t>(buffer[17]) |
                        (static_cast<uint16_t>(buffer[18]) << 8);
    return true;
  }
};
